...
void generate_next_ca(int rule, int ic, int stage) {
    ...
    // Asynchonous updates all as previous stage except one
    for (i=0; i<SIZE; i++) {
        cas[ic][stage][i] = cas[ic][stage-1][i];
    }
    // pick one randomly and update according to rule table    
    i = my_rand(SIZE);
